...
/**
 * The collision the MUI blueprint's own review found: one element is both the target of a tour
 * step that advances on a click and the target of a hotspot. The marker is `position: fixed`
 * over the target's top-right corner, so in a browser `document.elementFromPoint` at the
 * marker's centre returns the marker, not the target.
 */
...
describe('Hotspots', () => {
  ...
  it('draws no marker and emits no hotspot:show for a target that is present but not rendered', async () => {
    ...
    // an all-zero rect, which is truthy. Drawing on that puts a marker pointing at nothing,
    // announces an impression for a marker nobody saw, and lets a click retire the hotspot
    // before the user ever met the element it explains.
    await act(async () => {
  ...
  it('closing on an outside click that lands nowhere focusable returns focus to the marker', async () => {
    ...
    // A plain div cannot itself hold focus, so without an explicit recovery the browser's own
    // blur-on-mousedown-outside would strand focus on document.body.
  ...
  it('carries the documented classes and data-guide-part attributes', async () => {
    const user = userEvent.setup()
    render(<Page />)
    const marker = await screen.findByRole('button', { name: /Filters/ })
    expect(marker).toHaveClass('guide-hotspot')
    expect(marker).toHaveAttribute('data-guide-part', 'hotspot')

    await user.click(marker)
    const dialog = await screen.findByRole('dialog', { name: 'Filters' })
    expect(dialog).toHaveClass('guide-hotspot-bubble')
    expect(dialog).toHaveAttribute('data-guide-part', 'hotspot-bubble')
    expect(dialog.querySelector('[data-guide-part="hotspot-title"]')).toHaveClass('guide-hotspot-title')
    expect(dialog.querySelector('[data-guide-part="hotspot-body"]')).toHaveClass('guide-hotspot-body')
    expect(screen.getByRole('button', { name: 'Close' })).toHaveClass('guide-button')
  ...
  it('carries the documented class and part for the tour button', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: /Filters/ }))
    await screen.findByRole('dialog', { name: 'Filters' })
    const tourButton = screen.getByRole('button', { name: 'Show me' })
    expect(tourButton).toHaveClass('guide-button', 'guide-button-primary')
    expect(tourButton).toHaveAttribute('data-guide-part', 'hotspot-tour')
  })
...
describe('Hotspots against a running tour', () => {
  ...
  it('brings the markers back once the tour is over', async () => {
    ...
    // Suppression during a tour is deference, not retirement: the hotspot was never opened, so
